...
export function pointer(data,p){let v=data;if(p==='')return {exists:true,value:v};for(const k of p.slice(1).split('/').map(x=>x.replace(/~1/g,'/').replace(/~0/g,'~'))){if(v===null||typeof v!=='object'||(Array.isArray(v)&&!/^(0|[1-9]\d*)$/.test(k))||!Object.hasOwn(v,k))return {exists:false};v=v[k]}return {exists:true,value:v}}
...
export async function execute(test,project,settings){
 ...
 case'text':{const text=utf8(bytes(c.path));contentRules(text);const lines=text===''?0:text.split(/\r\n|\r|\n/).length-(/[\r\n]$/.test(text)?1:0);evidence.lines=lines;if(c.minLines!==undefined)check('Mindestzeilen',lines>=c.minLines,lines,c.minLines);if(c.maxLines!==undefined)check('Maximalzeilen',lines<=c.maxLines,lines,c.maxLines);break}
 ...
 case'command':{if(!settings.allowCommands)throw Error('Kommandoausführung in Einstellungen deaktiviert');const cwd=scoped(root,c.cwd??'.');const start=performance.now();const result=await new Promise((resolve,reject)=>{const child=spawn(c.executable,c.args,{cwd,shell:false,windowsHide:true,stdio:['ignore','pipe','pipe']});const output={out:[],err:[]};let count=0,overflow=false;
 const consume=which=>chunk=>{if(overflow)return;count+=chunk.length;if(count>MAX_BYTES){overflow=true;reject(Error('Prozessausgabe überschreitet 8 MiB'));return}output[which].push(chunk)};
 child.stdout.on('data',consume('out'));child.stderr.on('data',consume('err'));child.on('error',reject);
 child.on('close',(code,signal)=>{if(overflow)return;try{resolve({exitCode:code,signal,stdout:utf8(Buffer.concat(output.out)),stderr:utf8(Buffer.concat(output.err))})}catch(error){reject(Error('Prozessausgabe ist kein gültiges UTF-8: '+error.message))}})});evidence.process={executable:c.executable,args:c.args,cwd,...result,durationMs:performance.now()-start};check('Exitcode',c.expectedExitCodes.includes(result.exitCode),result.exitCode,c.expectedExitCodes);contentRules(result.stdout);if(c.maxDurationMs!==undefined)check('Prozessdauer',evidence.process.durationMs<=c.maxDurationMs,evidence.process.durationMs,c.maxDurationMs);break}
